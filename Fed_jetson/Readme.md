...
- You need to run 'scptrial.py' OUTSIDE the docker. The talk.txt file will server as messanger between py_process.py and 'scp_trial.py'. scptrial.py will do the task of file transfer. Make sure that you change the file names of the destination, when you copy these files to second client. For example, 'net1.pt' in one client and 'net2.pt' for other clients
- make sure to run py_process.py file only after running the server socket opening cell in server code notebooj
